...
Vector3d qp_startVel;
Vector3d qp_startAcc;
#define inf 1 >> 30
...
void TrajectoryGenerator::setLamda(double &lamda)
{
    this->_lamda = lamda;
}

Eigen::MatrixXd
TrajectoryGenerator::PolyQPGeneration(const Eigen::MatrixXd &Path, const Eigen::Vector3d &Vel,
                                      const Eigen::Vector3d &Acc, const Eigen::VectorXd &Time,
                                      const int &type)  // 1 for using minumum snap for intial; 2
                                                        // for using straight line for initial ;3
                                                        // for minimun arc length; 4 for minimum acc
{
    /*   Get initial trajectory which is a straight line( 0 zero end velocity and acceleration )
     * minimum snap trajectory or truly minumum snap trajectory  */
    ...
    for(int k = 0; k < m; k++)
    {
        for(int i = 3; i < 6; i++)
        {
            for(int j = 3; j < 6; j++)
            {
                H(k * 6 + i, k * 6 + j) = i * (i - 1) * (i - 2) * j * (j - 1) * (j - 2) /
                                          (i + j - 5) * pow(Time(k), (i + j - 5));
            }
        }
    }
    ...
    {                       // generating minumum snap curve
        ...
        // The Hessian of the total cost function
        MatrixXd Q = Eigen::MatrixXd::Zero(6 * m, 6 * m);
        ...
        {
            // Produce the Minimum arc length cost function
            for(int k = 0; k < m; k++)
            {
                for(int i = 1; i < 6; i++)
                {
        ...
        {
            // Produce the Minimum acc cost function
            for(int k = 0; k < m; k++)
            {
                for(int i = 2; i < 6; i++)
                {
                    for(int j = 2; j < 6; j++)
                    {
                        double fi = double(i);
                        double fj = double(j);
                        Q(k * 6 + i, k * 6 + j) = fi * (fi - 1) * fj * (fj - 1) / (fi + fj - 3);
                    }
                }
            }
        }
        ...
        {
            MatrixXd Qal = Eigen::MatrixXd::Zero(6 * m, 6 * m);
            MatrixXd Qacc = Eigen::MatrixXd::Zero(6 * m, 6 * m);
            for(int k = 0; k < m; k++)
            {
                for(int i = 1; i < 6; i++)
                {
            ...
            for(int k = 0; k < m; k++)
            {
                for(int i = 2; i < 6; i++)
                {
                    for(int j = 2; j < 6; j++)
                    {
                        double fi = double(i);
                        double fj = double(j);
                        Qacc(k * 6 + i, k * 6 + j) = fi * (fi - 1) * fj * (fj - 1) / (fi + fj - 3);
                    }
                }
            }
            Q = Qacc + _lamda * Qal;
    ...
    {                       // generating piecewise straight line
...
void TrajectoryGenerator::StackOptiDep()
{
    ...
    //----------------------generate arc length matrix-------------------------
    _Ql = MatrixXd::Zero(m * 6, m * 6);
    for(int k = 0; k < m; k++)
    {
        for(int i = 1; i < 6; i++)
        {
            for(int j = 1; j < 6; j++)
            {
                double fi = double(i);
                double fj = double(j);
                _Ql(k * 6 + i, k * 6 + j) = fi * fj / (fi + fj - 1);
            }
        }
    }
    _Rl = _C * B.transpose() * _Ql * (_A.inverse()) * Ct;

    _Rffl.resize(6, 6);
    _Rfpl.resize(6, 3 * m - 3);
    _Rpfl.resize(3 * m - 3, 6);
    _Rppl.resize(3 * m - 3, 3 * m - 3);

    _Rffl = _Rl.block(0, 0, 6, 6);
    _Rfpl = _Rl.block(0, 6, 6, 3 * m - 3);
    _Rpfl = _Rl.block(6, 0, 3 * m - 3, 6);
    _Rppl = _Rl.block(6, 6, 3 * m - 3, 3 * m - 3);
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> TrajectoryGenerator::getInitialD()
{
    ...
    Dxf(0) = _Dx(0);
    Dxf(3) = _Dx(_Dx.size() - 5);
    Dyf(0) = _Dy(0);
    Dyf(3) = _Dy(_Dy.size() - 5);
    Dzf(0) = _Dz(0);
    Dzf(3) = _Dz(_Dz.size() - 5);
...
Eigen::MatrixXd TrajectoryGenerator::getRl()
{
    return _Rl;
}
Eigen::MatrixXd TrajectoryGenerator::getRffl()
{
    return _Rffl;
}
Eigen::MatrixXd TrajectoryGenerator::getRppl()
{
    return _Rppl;
}
Eigen::MatrixXd TrajectoryGenerator::getRpfl()
{
    return _Rpfl;
}
Eigen::MatrixXd TrajectoryGenerator::getRfpl()
{
    return _Rfpl;
}