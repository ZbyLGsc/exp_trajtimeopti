...
int m; // number of segments in the trajectory
...
Eigen::MatrixXd TrajectoryGenerator::PolyQPGeneration(
            ...
            double &qp_cost) 
{     
      /*  quickly get a tiny minimum snap trajectory, using uniform time allocation  */
      Vel = init_s.vel;
      Acc = init_s.acc;
      m = Path.rows() - 1;

      /*cout<<"segment num: "<<m<<endl;
      cout<<"path :\n"<<Path<<endl;
      cout<<"init vel: \n"<<Vel<<endl;
      cout<<"init acc: \n"<<Acc<<endl;*/
      ...
      for(int k = 0; k < m; k++){
          ...
          for(int i = 0; i < 3; i++){
              ...
              for(int j = i; j < 6; j++)
                  Ab( 2 * i + 1, j ) = Factorial(j) / Factorial( j - i ) * pow( time, j - i );
          }
      ...
      for(int k = 0; k < m; k ++){
          for(int i = 3; i < 6; i ++){
              for(int j = 3; j < 6; j ++){
                  H( k*6 + i, k*6 + j ) = i * (i - 1) * (i - 2) * j * (j - 1) * (j - 2) / (i + j - 5) * pow( time, (i + j - 5) );
              }
          }
      }
      ...
      MatrixXd Q = H; // The Hessian of the total cost function, Now only minumum snap is used in the cost
...
/*      cout<<"Px: \n"<<Px<<endl;
      ...
      /*VectorXd Poly = Px + Py + Pz;
      qp_cost = Poly.transpose() * Q * Poly;
      
      cout<<qp_cost<<endl;*/
      qp_cost = (Px.transpose() * Q * Px + Py.transpose() * Q * Py + Pz.transpose() * Q * Pz)(0);
      //cout<<qp_cost<<endl;

/*      cout<<"cost : \n"<<Px.transpose() * Q * Px + Py.transpose() * Q * Py + Pz.transpose() * Q * Pz<<endl;
      cout<<"cost in d: \n"<<Dx1.transpose() * R * Dx1 + Dy1.transpose() * R * Dy1 + Dz1.transpose() * R * Dz1<<endl;*/
